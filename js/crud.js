...
window.addEventListener("click", (e)=>{
    ...
    if(e.target.matches("td"))
    {
        ...
        botonGuardar.innerHTML = '<i class="fas fa-save"></i>Modificar';
    ...
    else if (e.target.matches(".boton-guardar")) {
...
$formulario.addEventListener("submit", (e)=>{
    ...
    {
        if(txtId.value === ""){
            const nuevoMonstruo = new Monstruo(Date.now(), txtNombre.value, parseInt(rangeMiedo.value), txtAlias.value, rdoDefensa.value, selectTipo.value);
    
            handlerCrear(nuevoMonstruo);
        }
        else{
...
function handlerCrear(monstruo){
    ...
    GuardarData("monsterRotelli", monstruos);
...
function handlerActualizar(monstruoActualizado){
    let index = monstruos.findIndex((monstruo)=> monstruo.id == monstruoActualizado.id);
...
function handlerEliminar(id){
    let index = monstruos.findIndex((monstruo)=> monstruo.id == id);
    if(index != -1)
    {
